"""Module providing classes useful for the training procedure."""
import os
...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha: np.ndarray, gamma: float = 1.) -> None:
        ...
        easy examples to classify and upweights hard examples.
    ...
    def forward(self, predictions: torch.Tensor, targets: torch.Tensor):
        ...
        - the vector `alpha_t` which is based on the vector of frequencies of
        labels `alpha`
        ...
        -------
        Tensor
            The averaged focal loss score among all the instances of the
            minibatch.
        """
...
class Checkpoint():
    ...
    def __init__(self, model: nn.Module, path: Optional[str] = None) -> None:
        ...
        self.path = path if path is not None \
            else os.path.join('checkpoints', 'best_model.pth')

        os.makedirs(os.path.dirname(self.path), exist_ok=True)

    def save_best(self, new_metric: float, **kwargs) -> None:
        """Possibly save the best model weights in the checkpoint according 
        to the new value of the metric. Parameters defined in `kwargs` are 
        also saved in the checkpoints as an ndarray. 

        Parameters
        ----------
        new_metric : float
            The new value of the metric. It is compared to the best obtained
            so far and the checkpoints are updated solely if it is better.
        kwargs : Any
...
class EarlyStopping():
    ...
    def __init__(self, patience: int = 5, tolerance: float = .5) -> None:
        """Create an instance of an early stopping monitor.
        It saves the early stopping condition inside a boolean variable and
        updates it when asked by the user.

        Parameters
        ----------
        patience : int, optional
            Integer specifying how many times the result can get worse before
            the early stopping condition is triggered, by default 5
        tolerance : float, optional
            Float representing the tolerance considered for the comparison
            between the previous best result and the new one.
            If the difference between the best previous metric and the new
            considered one is greater than this value the result is
            considered worse and the counter of "failures" is incremented,
            by default 0.5.
        """
    ...
    def update(self, metric: float) -> None:
        """Update the state of the early stopping based on the new metric 
        value. If the difference between the previous saved best metric and
        the new one is less or equal than the tolerance, the counter of
        "failures" resets.
        Moreover, if the new metric is better than the previous one the 
        current value is saved as the best one.
        Otherwise, in the case the difference is greater than tolerance, 
        the counter of "failures" is incremented and, if it reaches the 
        patience value, the stop condition is triggered.
