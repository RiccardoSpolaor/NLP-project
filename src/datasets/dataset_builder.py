...
def get_dataframes(data_folder: str, df_type: Literal['training', 'validation']) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    assert df_type in ['training', 'validation'], 'Specify the `df_type` as either training or validation'
...
def split_dataframes(arguments_df: pd.DataFrame, labels_df: pd.DataFrame, seed: int = 42, 
    ...
    ((DataFrame, DataFrame), (DataFrame, DataFrame))
        A tuple containing:
        - The split arguments train dataframe and the respective labels dataframe;
        - The split arguments validation dataframe and the respective labels dataframe.
    """
